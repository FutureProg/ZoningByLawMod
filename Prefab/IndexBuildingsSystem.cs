...
namespace Trejak.ZoningByLaw.Prefab
{
    ...
    public partial class IndexBuildingsSystem : GameSystemBase
    {
        ...
        private List<ThemePrefab> _themePrefabs;
        ...
        protected override void OnCreate()
        {
            ...
            _themePrefabs = new List<ThemePrefab>();
        ...
        private void UpdateIndex(bool onUpdate)
        {
            ...
            foreach (ArchetypeChunk chunk in chunks)
            {
                ...
                for (int i = 0; i < buildingEntities.Length; i++)
                {
                    ...
                    // Theme is read the same way (building -> zone via SpawnableBuildingData.m_ZonePrefab)
                    // but ThemeObject.m_Theme is a single reference rather than an array, so a building has
                    // at most one theme. This mod strips ThemeObject off its own generated by-law zone
                    // (see Utils.cs), so this always reads the theme of the candidate building/zone being
                    // indexed, never the by-law zone itself.
                    ThemePrefab buildingTheme = null;
                    if (prefab.TryGet<ThemeObject>(out var buildingThemeObject) && buildingThemeObject.m_Theme != null)
                    {
                        buildingTheme = buildingThemeObject.m_Theme;
                    }
                    if (buildingTheme == null && SystemAPI.HasComponent<SpawnableBuildingData>(buildingEntity))
                    {
                        var zonePrefabEntity = SystemAPI.GetComponent<SpawnableBuildingData>(buildingEntity).m_ZonePrefab;
                        if (zonePrefabEntity != Entity.Null)
                        {
                            var zonePrefabBase = _prefabSystem.GetPrefab<PrefabBase>(zonePrefabEntity);
                            if (zonePrefabBase != null && zonePrefabBase.TryGet<ThemeObject>(out var zoneThemeObject) && zoneThemeObject.m_Theme != null)
                            {
                                buildingTheme = zoneThemeObject.m_Theme;
                            }
                        }
                    }
                    if (buildingTheme != null && !_themePrefabs.Contains(buildingTheme))
                    {
                        _themePrefabs.Add(buildingTheme);
                    }
                    ...
                    var props = new BuildingByLawProperties()
                    {
                        ...
                        // since most buildings belong to no pack/theme - EvalAssetStyle treats an
        ...
        public List<ThemePrefab> GetThemes()
        {
            return _themePrefabs;
        }

        public List<int> GetThemeHashes()
        {
            return _themePrefabs.Select(t => ThemeHashUtils.NameToHash(t.name)).ToList();
        }

        public ThemePrefab GetThemeByHash(int hash)
        {
            return _themePrefabs.Find(t => ThemeHashUtils.NameToHash(t.name) == hash);
        }

        public int GetThemeHash(ThemePrefab theme)
        {
            return ThemeHashUtils.NameToHash(theme.name);
        }
    ...
    public struct BuildingByLawProperties
    {
        ...
        public NativeArray<int> themes; // 32-bit hash of the building's theme name; empty when the building has no theme
    }
